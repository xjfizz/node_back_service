const sqlMap = {
  user: {
    ...
    // 新增用户
    add: 'insert IGNORE into user (name, password, email, create_time, update_time, type) values (?, ?, ?, ?, ?, ?)',
    // 更新用户信息
    updateInfo: 'update user set name = ?, password = ?, email = ?, update_time = ? where id = ?  and state != 0',
    // 更改用户状态
  ...
  goodsDetail: {
    getGoodsImgList: 'SELECT * FROM goods_detail_img WHERE userId = ?'
  }
}
