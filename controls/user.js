...
function formatData(rows) {
  return rows.map(row => {
    if(row.create_time) {
      row.createTime = $time.formatTime(row.create_time)
      delete row.create_time
    }
    if(row.update_time) {
      row.updateTime = $time.formatTime(row.update_time)
  ...
  add (req, res) {
    ...
    $http.userVerify(req, res, () => {
      ...
      if(!name || !password) $http.writeJson(res, {code: 2, message:'参数有误'})
        ...
        let arrayParams = [name, password, email, create_time, update_time, type]
        console.log('arrayParams', arrayParams)
        $http.connPool(sql, arrayParams, (err, result) => {
  ...
  list (req, res) {
    ...
    $http.userVerify(req, res, () => {
      ...
      if(!pageNum) {
        ...
        sql += " order by update_time desc limit ?,?"; // id倒序排
