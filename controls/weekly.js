...
const weekly = {
  ...
  list (req, res) {
    ...
    $http.userVerify(req, res, () => {
      ...
      if(searchStartTime){
        sqlSelectTotal += " and weekly.startTime >= "+ " ' " + `${searchStartTime}` + " ' "
        sqlSelectList += " and weekly.startTime >= "+ " ' " + `${searchStartTime}` + " ' "
      }
      if(searchEndTime){
        sqlSelectTotal += " and weekly.endTime <= "+ " ' " + `${searchEndTime}` + " ' "
        sqlSelectList += " and weekly.endTime <= "+ " ' " +  `${searchEndTime}` + " ' "
      }

      let sql= sqlSelectTotal + '; ' + sqlSelectList
      sql += " order by weekly.id desc limit ?,?"; // id倒序排
      let arrayParams = [limitFirst, limitLast]
      console.log('sql:', sql, arrayParams)
      $http.connPool(sql, arrayParams, (err, result) => {
        console.log(result)
        if(err) {
